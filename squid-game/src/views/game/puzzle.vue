      ...
      <div id="game-group">
        ...
        >
          <transition-group
            ...
            tag="div"
            :style="{
              gridTemplateColumns: `repeat(${ratio}, 1fr)`,
              gridTemplateColumns: `repeat(${ratio}, 1fr)`,
            }"
          >
            <button
              ...
              :key="tile.val"
              @keyup.prevent="handleArrow"
              :index="index"
              :ref="!tile.val && 'empty'"
              :disabled="!tile.isPossibleMove && tile.val > 0"
              class="tile"
              :class="{
                correct: isTileCorrect(tile.val, index),
                possible_move: tile.isPossibleMove,
              }"
          ...
          <transition name="fade">
            <div v-if="!gameStarted" class="loader">
              <p>
                Randomizing solvable puzzle<span class="dot">.</span
        ...
        <p id="counter">
          <span
            id="progress-bar"
            :style="{ width: (howManyCorrect / ratioSquared) * 100 + '%' }"
          ></span>
      ...
      <aside id="options">
        <div id="custom-image">
          ...
          <select v-model="imageSelect" name="imageSelect" id="imageSelect">
            <option value="../images/puzzle/KKANBU_rabbit1.jpeg">
              KKANBU_rabbit1
            </option>
            <option value="../images/puzzle/KKANBU_rabbit2.jpeg">
              KKANBU_rabbit2
            </option>
            <option value="../images/puzzle/KKANBU_rabbit3.jpeg">
              KKANBU_rabbit3
            </option>
            <option value="../images/puzzle/KKANBU_rabbit4.jpeg">
              KKANBU_rabbit4
            </option>
          </select>

          <input
            v-if="imageSelect === `custom`"
            @click="highlightInput"
            v-model="customImage"
...
export default {
  data() {
    return {
      ...
      invertNumbers: false,
      dimTiles: false,
      imageSelect: "../images/puzzle/KKANBU_rabbit1.jpeg",
      customImage: "../images/puzzle/KKANBU_rabbit4.jpeg",
      showSolution: true,
      gameStarted: false,
      time: 0,
      isRunning: false,
  ...
  watch: {
      ...
      if (this.gameStarted) {
        ...
        this.$nextTick(() => {
          ...
          newTiles.forEach((tile, index) => {
            newTiles[index].isPossibleMove = legalMoves.includes(index)
              ? true
              : false;
          });
        });
    ...
    customImage() {
      document
        .querySelector("#puzzleEntire")
        .style.setProperty("--backgroundImage", `url(${this.customImage})`);
    },
    ...
    imageSelect() {
      document
        ...
        .style.setProperty(
          "--backgroundImage",
          this.imageSelect === "custom"
            ? `url(${this.customImage})`
  ...
  created() {
    ...
    for (let i = 0; i < max; i++) {
      this.tiles.push(
        i === max - 1
          ? { val: "", isPossibleMove: false }
          : { val: i + 1, isPossibleMove: false }
      );
      this.solution.push(
        i === max - 1
          ? { val: "", isPossibleMove: false }
  ...
  mounted() {
    ...
    document
      .querySelector("#puzzleEntire")
      .style.setProperty("--ratio", this.ratio);
    document
      .querySelector("#puzzleEntire")
      .style.setProperty(
        "--backgroundImage",
  ...
  computed: {
    ...
    howManyCorrect() {
      const correctlyPlacedTiles = this.tiles.filter((tile, index) => {
        return (
          Number(tile.val) == Number(index + 1) ||
          (Number(index + 1) == this.ratioSquared && !tile.val)
        );
      });

      if (
        correctlyPlacedTiles.length === this.ratioSquared &&
        this.gameStarted === true
      ) {
        setTimeout(() => {
          alert(
            'YOU WIN! Click the "re-shuffle" button to start a new game (or adjust the ratio to get crazy)!' +
              " Time : " +
              this.time
          );
          this.stopTimer();
  ...
  methods: {
    ...
    getBackgroundPosition(val) {
      return val
        ? `${(100 / (this.ratio - 1)) * (val - 1)}% ${
            Math.floor((val - 1) / this.ratio) * (100 / (this.ratio - 1))
          }%`
        : "5% 5%";
    },
    ...
    moveIsNotCrossRowHorizontal(a, b) {
      ...
      return (
        ((a - b === 1 || b - a === 1) && !this.illegalMoves.includes(a + b)) ||
        (a - b !== 1 && b - a !== 1)
      );
    },
    ...
    isValidMove(a, b) {
      ...
      b = Number(b);
      if (
        this.moveIsNotTheSameTile(a, b) &&
        this.moveIsInBounds(a, b) &&
        this.moveIsAdjacentTile(a, b) &&
        ...
        if (randomized > 0) {
          ...
          if (!this.isValidMove(a, b)) {
            ...
            if (randomized > 0) {
              ...
              document
                .querySelector("#puzzleEntire")
                .style.setProperty("--transition", "transform .15s ease-out");
              return;
            }
          }
        }
    ...
    getAccessibleTilePosition(val, index) {
      ...
      return `${tileIdentifier} ${
        val - 1 === index ? "correctly placed" : ""
      } in position ${index + 1}: row ${
        Math.floor(index / this.ratio) + 1
      }, column ${(index % this.ratio) + 1}`;
    },
    ...
    getEmptyTileIndex() {
      return this.$refs.empty
        ? Number(this.$refs.empty[0].getAttribute("index"))
        : this.ratioSquared - 1;
    },
...
<style scoped src="../../assets/css/game/puzzle.css"></style>
